<template>
  <div>
    ...
    <b-card-text>
      <v-chart ref="chart" :options="chartOptions" class="pb-5" />
      <v-chart ref="chart2" :options="chartOptions2" class="pb-5" />
    </b-card-text>
...
export default {
  ...
  computed: {
    ...
    chartOptions2() {
      ...
      return {
        ...
        xAxis: {
          ...
          min: that.minDate,
          max: that.maxDate,
         // min: function(){let date = new Date(); date.setHours(0,0,0,0); return date;},
          axisLine: {
            show: true
          }
        },
    ...
    dataComputed() {
      return this.dataChart.map(function(item, idx) {
        let date = new Date();
        console.log(idx % 24);
        date.setHours(idx % 24);
        return [date, item[0], item[2]];
      });
    }
  },
  data() {
    return {
      ...
      hours: [...Array(24).fill(0).map((k,idx)=>{ let date = new Date(); date.setHours(idx,0,0,0); return date;} )],
      dataChart: [
        [0, 0, 5],
        [0, 1, 1],
        [0, 2, 0],
  ...
  mounted() {
    ...
    for (const _index of Array(300).keys()) {
     // console.log(_);
            this.chartOptions.series[Math.floor(Math.random() * 5)].data.push([
